...
const Message = ({ message, own, mk, type }) => {
    ...
    function linkify(str) {
        ...
        let codeIndex = str.indexOf('videoChat');
        let roomCode = '';

        if (codeIndex !== -1) {
            const t = str.split('/');
            roomCode = t[t.length - 1];
        }
        while ((match = RE_URL.exec(str))) {
            ...
            results.push(
                <a key={results.length} onClick={() => join(roomCode, roomType)} target="_blank">
                    {link}
                </a>
            );
                        ...
                        <Grid item className={classes.message}>
                            ...
                            {message.sender === 'CHAT' && (
                                <ListItemText align="center">
                                    <Typography variant="overline">{message.text}</Typography>
                                </ListItemText>
                            )}
